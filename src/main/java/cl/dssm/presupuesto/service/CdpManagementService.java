...
public class CdpManagementService {
    ...
    public Cdp create(CdpRequestDto input) {
        if (cdpRepository.findByCdpNumber(input.cdpNumber()).isPresent()) {
    ...
    public Cdp update(Long id, CdpRequestDto input) {
        ...
        cdpRepository.findByCdpNumber(input.cdpNumber()).ifPresent(existing -> {
            if (!existing.getId().equals(id)) throw new IllegalArgumentException("Ya existe otro CDP con el número " + input.cdpNumber());
        });
        apply(cdp, input);
        cdp = cdpRepository.save(cdp);
        cdp = recalculationService.recalculate(cdp);
