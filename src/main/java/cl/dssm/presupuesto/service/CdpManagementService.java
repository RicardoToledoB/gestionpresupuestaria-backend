...
public class CdpManagementService {
    ...
    private void apply(Cdp cdp, CdpRequestDto input) {
        ...
        cdp.setCoverageMonths(calculateCoverageMonths(input.coverageStart(), input.coverageEnd()));
    ...
    private BigDecimal calculateCoverageMonths(LocalDate start, LocalDate end) {
        if (start == null || end == null || end.isBefore(start)) {
            return BigDecimal.ZERO;
        }
        int months = ((end.getYear() - start.getYear()) * 12) + (end.getMonthValue() - start.getMonthValue()) + 1;
        return BigDecimal.valueOf(Math.max(months, 0));
    }
}
