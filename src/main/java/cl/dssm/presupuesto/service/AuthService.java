...
public class AuthService {
    ...
    public AuthResponseDto login(AuthRequestDto request) {
        ...
        if (!Boolean.TRUE.equals(user.getActive()) || user.getDeletedAt() != null) {
            auditLogService.register("SEGURIDAD", "LOGIN_BLOQUEADO", "AppUser", user.getId(), user.getUsername(), null, null, "Usuario inactivo o eliminado");
            throw new BadCredentialsException("Usuario inactivo o no autorizado.");
        }

        if (user.getLockedAt() != null) {
            auditLogService.register("SEGURIDAD", "LOGIN_BLOQUEADO", "AppUser", user.getId(), user.getUsername(), null, null, "Usuario bloqueado");
