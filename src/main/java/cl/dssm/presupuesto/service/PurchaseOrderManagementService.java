...
public class PurchaseOrderManagementService {
    ...
    public PurchaseOrder create(PurchaseOrderRequestDto input) {
        if (repository.findByOrderNumber(input.orderNumber()).isPresent()) {
    ...
    public PurchaseOrder update(Long id, PurchaseOrderRequestDto input) {
        ...
        repository.findByOrderNumber(input.orderNumber()).ifPresent(existing -> {
            if (!existing.getId().equals(id)) throw new IllegalArgumentException("Ya existe otra OC con el número " + input.orderNumber());
        });
        apply(po, input);
        po = repository.save(po);
    ...
    private String snapshot(PurchaseOrder po) {
        return "OC=" + po.getOrderNumber() + "; CDP=" + (po.getCdp() != null ? po.getCdp().getCdpNumber() : "-") + "; programa=" + (po.getProgram() != null ? po.getProgram().getName() : "-") + "; montoReal=" + po.getRealAmount();
    }
