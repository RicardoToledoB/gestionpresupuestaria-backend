...
public class PurchaseOrderManagementService {
    ...
    @Transactional
    public PurchaseOrder associatePurchaseRequest(Long id, String purchaseRequestId) {
        PurchaseOrder po = repository.findById(id).orElseThrow();
        String cleanId = purchaseRequestId == null ? null : purchaseRequestId.trim();
        if (cleanId == null || cleanId.isBlank()) {
            throw new IllegalArgumentException("Debe indicar el ID de solicitud de compra CeroPapel.");
        }
        String before = snapshot(po);
        po.setPurchaseRequestId(cleanId);
        po = repository.save(po);
        auditLogService.register("ORDENES_COMPRA", "ASOCIAR_SOLICITUD_COMPRA", "PurchaseOrder", po.getId(), po.getOrderNumber(), before, snapshot(po), "Asociación de OC con solicitud de compra CeroPapel " + cleanId);
        return po;
    }

    @Transactional
    public PurchaseOrder removePurchaseRequestAssociation(Long id) {
        PurchaseOrder po = repository.findById(id).orElseThrow();
        String before = snapshot(po);
        String previous = po.getPurchaseRequestId();
        po.setPurchaseRequestId(null);
        po = repository.save(po);
        auditLogService.register("ORDENES_COMPRA", "DESASOCIAR_SOLICITUD_COMPRA", "PurchaseOrder", po.getId(), po.getOrderNumber(), before, snapshot(po), "Desasociación de solicitud de compra CeroPapel " + (previous != null ? previous : "-"));
        return po;
    }
    ...
    private void apply(PurchaseOrder po, PurchaseOrderRequestDto input) {
        ...
        po.setStatus(input.status() == null ? PurchaseOrderStatus.SIN_ESTADO : input.status());
