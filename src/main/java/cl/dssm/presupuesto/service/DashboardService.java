    ...
    public List<ProgramSummaryDto> programs() {
        return programRepository.findAll().stream()
                ...
                .map(p -> {
                    ...
                    BigDecimal available = zero(p.getCurrentBudget()).subtract(issued);
                    BigDecimal pct = percent(issued, zero(p.getCurrentBudget()));
                    return new ProgramSummaryDto(p.getId(), p.getName(), zero(p.getCurrentBudget()), issued, available, pct);
                })
                .toList();
    }
