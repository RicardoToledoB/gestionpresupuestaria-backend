...
public class DashboardService {
    ...
    private String resolveSubtitle(BudgetProgram program) {
        if (program.getSubtitle() != null && !program.getSubtitle().isBlank() && !program.getSubtitle().equals("-")) {
            return program.getSubtitle();
        }
        String name = program.getName() == null ? "" : program.getName().toUpperCase();
        if (name.contains("SUBT 31") || name.contains("INICIATIVAS DE INVERSION")) return "31";
        if (name.contains("SUBT 29") || name.contains("ACTIVOS NO FINANCIEROS") || name.contains("PMEL")) return "29";
        return "22";
    }
