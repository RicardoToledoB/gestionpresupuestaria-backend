...
public class ExcelImportService {
    ...
    public ImportResultDto importWorkbook(MultipartFile file) {
        ...
        try (InputStream is = file.getInputStream(); Workbook wb = new XSSFWorkbook(is)) {
            ...
            if (ocSheet != null) {
                ...
                // Importante: no recalculamos CDP al importar desde Excel.
                // La hoja CDP ya contiene los montos ejecutados, saldos, estados y posible liberación
                // validados por la matriz original. Si recalculamos desde OC, los totales dejan de cuadrar
                // porque el Excel maneja líneas repetidas de CDP/OC y no solo registros únicos.
            }

            Sheet resumenSheet = wb.getSheet("Resumen");
            if (resumenSheet != null) {
                importResumenProgramData(resumenSheet, excelImport, errors);
                programs = (int) programRepository.count();
            }
        } catch (Exception ex) {
    ...
    private void importResumenProgramData(Sheet sheet, ExcelImport excelImport, List<ExcelImportError> errors) {
        // Hoja Resumen: tabla presupuestaria desde G6:O31.
        // Columnas reales:
        // G = Programa, H = St, I = Presupuesto vigente.
        // El campo St es oficial en el Excel y se usa en el dashboard/reporte.
        for (int i = 6; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                ...
                String st = ExcelReaderUtils.text(row, 7);
                BigDecimal currentBudget = ExcelReaderUtils.money(row, 8);
                ...
                if (st != null) {
                    program.setSubtitle(st);
                }
                if (currentBudget.compareTo(BigDecimal.ZERO) != 0) {
                    program.setCurrentBudget(currentBudget);
                    if (program.getInitialBudget() == null || program.getInitialBudget().compareTo(BigDecimal.ZERO) == 0) {
                        program.setInitialBudget(currentBudget);
                    }
                }
                programRepository.save(program);
            } catch (Exception ex) {
                errors.add(error(excelImport, "Resumen", i + 1, "WARN",
                        "Resumen presupuestario omitido para una fila por error de lectura/importación: " + ex.getMessage()));
            }
        }
    }
    ...
    private int importCdps(Sheet sheet, ExcelImport excelImport, List<ExcelImportError> errors) {
        ...
        for (int i = 3; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                ...
                String cdpTypeName = ExcelReaderUtils.text(row, 6);
                cdp.setCdpType(cdpTypeName);
                resolveCdpType(cdpTypeName);
    ...
    private int importPurchaseOrders(Sheet sheet, ExcelImport excelImport, List<ExcelImportError> errors) {
        ...
        for (int i = 3; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                ...
                Cdp cdp = cdpRepository.findFirstByCdpNumberAndDeletedAtIsNullOrderByIdAsc(ExcelReaderUtils.text(row, 7)).orElse(null);
                ...
                String rawStatus = ExcelReaderUtils.text(row, 13);
                PurchaseOrderStatus status = parsePurchaseOrderStatus(rawStatus);
                resolvePurchaseOrderState(rawStatus, status);
                po.setStatus(status);
                po.setObservation(ExcelReaderUtils.text(row, 14));
                String subtitle = ExcelReaderUtils.text(row, 15);
                po.setSubtitle(subtitle);
                resolveBudgetSubtitle(subtitle);
    ...
    private CdpType resolveCdpType(String rawName) {
        if (rawName == null || rawName.isBlank()) return null;
        String name = rawName.trim();
        return cdpTypeRepository.findByNameIgnoreCase(name)
                .orElseGet(() -> cdpTypeRepository.save(CdpType.builder()
                        .name(name)
                        .code(normalizeCode(name))
                        .description("Detectado automáticamente desde hoja CDP")
                        .active(true)
                        .build()));
    }
    ...
    private PurchaseOrderStatus parsePurchaseOrderStatus(String value) {
        if (value == null || value.isBlank()) return PurchaseOrderStatus.SIN_ESTADO;
        String normalized = normalizeCode(value);
        if (normalized == null || normalized.isBlank()) return PurchaseOrderStatus.SIN_ESTADO;
        try {
            return PurchaseOrderStatus.valueOf(normalized);
        } catch (Exception ex) {
            // Si en el Excel aparece un estado no contemplado en el enum, de todas formas se carga
            // en el mantenedor por resolvePurchaseOrderState(...) y la OC queda con estado técnico SIN_ESTADO.
            return PurchaseOrderStatus.SIN_ESTADO;
        }
    }

    private PurchaseOrderState resolvePurchaseOrderState(String rawName, PurchaseOrderStatus status) {
        String name = rawName == null || rawName.isBlank() ? "Sin Estado" : rawName.trim();
        String code = status == null ? normalizeCode(name) : status.name();
        if (code == null || code.isBlank()) code = "SIN_ESTADO";
        final String safeCode = code;
        final String safeName = name;
        return purchaseOrderStateRepository.findByCodeIgnoreCase(safeCode)
                .or(() -> purchaseOrderStateRepository.findByNameIgnoreCase(safeName))
                .map(existing -> {
                    existing.setCode(safeCode);
                    existing.setName(safeName);
                    existing.setDescription("Detectado automáticamente desde hoja OC");
                    existing.setActive(true);
                    existing.setDeletedAt(null);
                    return purchaseOrderStateRepository.save(existing);
                })
                .orElseGet(() -> purchaseOrderStateRepository.save(PurchaseOrderState.builder()
                        .code(safeCode)
                        .name(safeName)
                        .description("Detectado automáticamente desde hoja OC")
                        .active(true)
                        .build()));
    }

    private BudgetSubtitle resolveBudgetSubtitle(String rawCode) {
        String code = rawCode == null || rawCode.isBlank() ? "SIN_SUBTITULO" : rawCode.trim();
        String name = code.equals("SIN_SUBTITULO") ? "Sin subtítulo" : "Subtítulo " + code;
        return budgetSubtitleRepository.findByCodeIgnoreCase(code)
                .map(existing -> {
                    existing.setName(existing.getName() == null || existing.getName().isBlank() ? name : existing.getName());
                    existing.setDescription("Detectado automáticamente desde hoja OC");
                    existing.setActive(true);
                    existing.setDeletedAt(null);
                    return budgetSubtitleRepository.save(existing);
                })
