...
public interface CdpRepository extends JpaRepository<Cdp, Long> {
    ...
    @Query("""
            select coalesce(sum(c.pendingBalance), 0) from Cdp c
            where c.deletedAt is null
              and c.program.id = :programId
              and lower(coalesce(c.cdpType, '')) <> 'oc directa'
            """)
    BigDecimal sumPendingBalanceByProgramIdExcludingDirectOc(@Param("programId") Long programId);

    @Query("select coalesce(sum(c.possibleReleaseAmount), 0) from Cdp c where c.deletedAt is null and c.program.id = :programId")
    BigDecimal sumPossibleReleaseAmountByProgramId(@Param("programId") Long programId);

    @Query("select count(c.id) from Cdp c where c.deletedAt is null and c.program.id = :programId and c.alertStatus = :status")
    long countByProgramIdAndAlertStatus(@Param("programId") Long programId, @Param("status") AlertStatus status);

    @Query("""
            select count(c.id) from Cdp c
            where c.deletedAt is null
              and c.program.id = :programId
              and c.alertStatus in :statuses
            """)
    long countByProgramIdAndAlertStatusIn(@Param("programId") Long programId, @Param("statuses") List<AlertStatus> statuses);
