...
public interface CdpRepository extends JpaRepository<Cdp, Long> {
    Optional<Cdp> findByCdpNumber(String cdpNumber);
    long countByAlertStatus(AlertStatus status);

    @Query("select count(c.id) from Cdp c where c.deletedAt is null")
    long countActive();

    @Query("select count(c.id) from Cdp c where c.program is null")
    long countWithoutProgram();

    @Query("select count(c.id) from Cdp c where c.provider is null")
