...
public class UserController {
  ...
  @PostMapping public AppUser create(@RequestBody AppUser e){
    e.setId(null);
    String raw = e.getPasswordHash();
    e.setPasswordHash(passwordEncoder.encode(raw == null || raw.isBlank() ? "Cambiar123" : raw));
    return repository.save(e);
  }

  @PutMapping("/{id}") public AppUser update(@PathVariable Long id,@RequestBody AppUser i){
    AppUser e=repository.findById(id).orElseThrow();
    e.setUsername(i.getUsername()); e.setFullName(i.getFullName()); e.setEmail(i.getEmail()); e.setRolesText(i.getRolesText()); e.setActive(i.getActive());
    if (i.getPasswordHash()!=null && !i.getPasswordHash().isBlank()) e.setPasswordHash(passwordEncoder.encode(i.getPasswordHash()));
    return repository.save(e);
  }
