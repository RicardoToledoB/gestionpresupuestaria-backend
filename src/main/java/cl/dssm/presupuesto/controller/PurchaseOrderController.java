...
import cl.dssm.presupuesto.dto.AssociatePurchaseRequestDto;
...
public class PurchaseOrderController {
    ...
    @Transactional(readOnly = true)
    public java.util.List<AssociatedPurchaseOrderDto> findByPurchaseRequest(@PathVariable String purchaseRequestId) {
        return repository.findByPurchaseRequestIdAndDeletedAtIsNullOrderByOrderDateDescOrderNumberAsc(purchaseRequestId.trim())
                .stream()
                .map(AssociatedPurchaseOrderDto::fromEntity)
                .toList();
    }

    @PostMapping("/{id}/purchase-request-association")
    public PurchaseOrderResponseDto associatePurchaseRequest(@PathVariable Long id, @RequestBody @Valid AssociatePurchaseRequestDto input) {
        return PurchaseOrderResponseDto.fromEntity(service.associatePurchaseRequest(id, input.purchaseRequestId()));
    }

    @DeleteMapping("/{id}/purchase-request-association")
    public PurchaseOrderResponseDto removePurchaseRequestAssociation(@PathVariable Long id) {
        return PurchaseOrderResponseDto.fromEntity(service.removePurchaseRequestAssociation(id));
    }
